...
local box_END is wndw:addhlayout().
	...
	local ENDvalue is box_END:ADDTEXTFIELD("250000").
...
local startTime is time:seconds + nextnode:eta - (ff_Burn_Time(nextnode:deltaV:mag, 258, 1.5, 1) / 2).
Print "burn starts at: " + startTime.
wait 5.
warpto(startTime - 75).
wait until time:seconds > startTime - 70.
...
function hf_APScore{
  parameter mnv.
	Local result is -(abs(Body:GEOPOSITIONOF(positionAT(ship, hf_TimeToApoapsis(mnv))):lat)*10000) - abs(mnv:orbit:apoapsis -endheight).// - nextnode:deltav:mag.
	Print result.
  return result.
}
...
function hf_lowScore{
  parameter mnv, endPE is 35793172.
